...
(function( $ ){
  ...
  var methods = {
     init : function( options ) {
		...
		return this.each(function() {
			...
			if (settings.allowNewElements){
				selectElement.prepend('<option>Add new Field</option>');
			}
			...
			selectElement.keydown( function(e){
				...
				if( selected=="Add new Field" || settings['editableElements'] ) {
	 ...
	 enableAddingNewElements: function(){
		var fChild = $(this).children().first();
		if(fChild.text() != "Add new Field"){
			$(this).prepend('<option>Add new Field</option>');
		}
	 },
